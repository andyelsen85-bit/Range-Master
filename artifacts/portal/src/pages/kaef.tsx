    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="border-b border-border/50 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Meng Akeef</h1>
            <p className="mt-1 text-sm font-medium text-muted-foreground">
              Är perséinlech Kafgeschicht.
            </p>
          </div>
        </div>
      </header>
      ...
      {isLoading ? (
        <div className="space-y-3" aria-label="Kafgeschicht gëtt gelueden" data-testid="status-purchases-loading">
          {[1, 2, 3].map((item) => (
        ...
        <section className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center" aria-live="polite" data-testid="status-purchases-error">
          ...
          <h2 className="font-bold text-foreground">Kafgeschicht konnt net geluede ginn</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            {error instanceof Error ? error.message : "Probéiert wgl. nach eng Kéier."}
          </p>
          <button
            type="button"
            onClick={() => refetch()}
          ...
          >
            {isFetching ? "Lueden…" : "Nach eng Kéier probéieren"}
          </button>
        </section>
      ) : purchases.length === 0 ? (
        <section className="rounded-xl border border-border/50 bg-card p-12 text-center" data-testid="status-purchases-empty">
          ...
          <h2 className="font-bold text-foreground">Nach keng Akeef</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Är ofgeschloss Akeef erschéngen hei.
          </p>
        </section>
      ) : (
        <>
          <div className="hidden overflow-x-auto rounded-xl border border-border/50 bg-card shadow-sm md:block">
            <table className="w-full text-left text-sm">
              <caption className="sr-only">Är perséinlech Kafgeschicht</caption>
              <thead className="bg-secondary/20 text-xs uppercase tracking-widest text-muted-foreground">
                <tr className="border-b border-border/50">
                  ...
                  <th scope="col" className="px-5 py-3 text-right font-bold">Quantitéit</th>
            ...
            {purchases.map((purchase) => (
              <article key={purchase.externalId} className="rounded-xl border border-border/50 bg-card p-4 shadow-sm" data-testid={`card-purchase-${purchase.externalId}`}>
                ...
                <dl className="mt-4 grid grid-cols-3 gap-3 border-t border-border/40 pt-3 text-sm">
                  <div>
                    <dt className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Quantitéit</dt>
